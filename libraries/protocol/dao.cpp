...
namespace graphene { namespace protocol {
    ...
    void dao_dapp_options::dao_dapp_options() const
    {
    ...
    void dao_dapp_create_operation::validate() const
    {
        common_options.validate();
        FC_ASSERT( fee.amount >= 0 );
        FC_ASSERT( is_valid_name( name ) );
    ...
    share_type dao_dapp_create_operation::calculate_fee( fee_parameters_type const& schedule ) const
    {
        auto core_fee_required = schedule.fee; 
        // common_options contains several lists and a string. Charge fees for its size
        core_fee_required += calculate_data_fee( fc::raw::pack_size( *this ), schedule.price_per_kbyte );
...
GRAPHENE_IMPLEMENT_EXTERNAL_SERIALIZATION( graphene::protocol::dao_dapp_options )
...
GRAPHENE_IMPLEMENT_EXTERNAL_SERIALIZATION( graphene::protocol::dao_dapp_create_operation::fee_parameters_type )
...
GRAPHENE_IMPLEMENT_EXTERNAL_SERIALIZATION( graphene::protocol::dao_dapp_create_operation )